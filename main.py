...
if st.button('Predict Cluster'):
    ...
    try:
        data_baru['Gender'] = data_baru['Gender'].apply(lambda x: encoder['Gender'].transform([x])[0])
    except ValueError:
        st.error("Nilai 'Gender' tidak dikenali oleh encoder.")
        st.stop()
        
    data_baru['Physical Activity Level'] = data_baru['Physical Activity Level'].replace({
        "Sedentary": 0, "Lightly Active": 1, "Moderately Active": 2, "Very Active": 3
    })
    data_baru['Sleep Quality'] = data_baru['Sleep Quality'].replace({
    ...
    if cluster_pred[0] == 0:
        ...
        cluster_label = 'Penambahan berat badan dengan tingkat aktivitas fisik lebih tinggi'
    elif cluster_pred[0] == 3:
        cluster_label = 'Penurunan berat badan dengan aktivitas fisik lebih aktif'
    ...
    cluster_labels = {
        ...
        2: 'kelompok dengan fokus pada menjaga berat badan stabil',
        3: 'kelompok dengan fokus pada pola makan dan olahraga optimal'
    }
